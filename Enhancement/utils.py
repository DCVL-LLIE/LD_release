...
# FID 관련 함수들
# -------------------------
def get_activations(folder, model, device, batch_size=32):
    model.eval()
    transform = T.Compose([
        T.ToPILImage(),
        T.Resize((299, 299)),
    ...
    files = [f for f in os.listdir(folder) if f.lower().endswith(('.png','.jpg','.jpeg','.bmp','.tiff'))]
    act_list = []
    ...
    with torch.no_grad():
        for i in range(0, len(files), batch_size):
            ...
            for fn in batch_files:
                img = load_img(os.path.join(folder, fn))
                img = transform(img)
                imgs.append(img)
            imgs = torch.stack(imgs, dim=0).to(device)
            # inception_v3의 aux_logits=False, transform_input=False
            pred = model.forward(imgs)
            # (N,2048,1,1) 형태 -> (N,2048)
            if pred.dim() == 4:
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    cov_sqrt, _ = linalg.sqrtm(sigma1.dot(sigma2), disp=False)
    if not np.isfinite(cov_sqrt).all():
        # 수치 안정화
        offset = np.eye(sigma1.shape[0]) * eps
        cov_sqrt = linalg.sqrtm((sigma1+offset).dot(sigma2+offset))
    # 실수부만 취함
    cov_sqrt = np.real(cov_sqrt)
    diff = mu1 - mu2
    return diff.dot(diff) + np.trace(sigma1 + sigma2 - 2*cov_sqrt)

def calculate_fid(gt_folder, test_folder, device):
    model = inception_v3(pretrained=True, transform_input=False).to(device)
    act1 = get_activations(gt_folder, model, device)
    act2 = get_activations(test_folder, model, device)
